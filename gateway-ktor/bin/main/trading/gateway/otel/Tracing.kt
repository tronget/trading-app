...
/**
 * Серверный спан на каждый HTTP-запрос. Контекст пробрасывается в корутины
 * через asContextElement, поэтому исходящие вызовы Data-сервиса наследуют
 * trace-id (inject в DataServiceClient).
 */
fun Application.installServerTracing() {
    val tracer = GlobalOpenTelemetry.getTracer("gateway")
